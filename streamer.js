(function () {
  ...
  startDetailPage("streamers", streamer, getPlaylistsByStreamer, renderStreamerPage);

  function renderStreamerPage(items, relatedFromData) {
    ...
    // 分割データには、この集計結果のうち10番目の件数以上の候補が入っている(並び順はここで決める)。
    let overlapEntries;
    if (relatedFromData) {
      overlapEntries = relatedFromData.map(([name, count]) => ({ name: name, count: count }));
    } else {
      ...
      overlapEntries = overlapOrder.map((name) => ({ name: name, count: overlapGames[name].size }));
    }
    const relatedStreamers = overlapEntries
      .sort((a, b) => b.count - a.count || a.name.localeCompare(b.name, "ja"))
      .slice(0, 10);
